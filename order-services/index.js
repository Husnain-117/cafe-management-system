...
app.post('/orders', async (req, res) => {
  ...
  try {
    ...
    if (customerId) {
      const customerResponse = await axios.get(`${process.env.CUSTOMER_SERVICES_URL}/customers/${customerId}`);
    ...
    const menuResponse = await axios.get(`${process.env.MENU_SERVICES_URL}/menu`);
    ...
    // Update inventory
    await axios.post(`${process.env.INVENTORY_SERVICES_URL}/inventory/update`, { items });

    // Update loyalty points if customer exists
    if (customerId) {
      await axios.post(`${process.env.CUSTOMER_SERVICES_URL}/customers/update-points`, {
        customerId,
